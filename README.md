...
- **托盘常驻**：全局热键触发，托盘菜单支持“最近一次”与快速切换（剪贴板模式/路径格式），并可打开设置/目录与退出。
...
- 路径格式：`Text` / `Markdown` / `HTML`
  - Markdown 示例：`![截图](<file:///C:/Path/to/image.png>)`
  - HTML 示例：`<img src="file:///C:/Path/to/image.png"/>`
- Markdown/HTML 输出：`仅片段` / `仅纯路径` / `片段+纯路径`（仅对路径格式为 Markdown/HTML 时生效）
...
> **重置默认提醒**：设置页的“重置默认”会将保存目录重置为默认目录 `Pictures\PathSnip`，并保留其它设置项的“保存后生效”语义。
...
- **v1.2.2**：设置保存语义与热键失败回滚进一步收口，并完成 Overlay / MainWindow / SettingsWindow 三处低风险 partial 解耦。
- **v1.2.1**：托盘支持“最近一次”与快速切换；Markdown/HTML 路径复制更稳定，并修复设置页崩溃与禁用态显示。
...
- 若使用“重置默认”，保存目录会回到 `Pictures\PathSnip`，请确认你的现有保存路径是否需要迁移。
